...
const ProfilePage = () => {
  ...
  const userProfile = useBoundStore((state) => state.userProfile);
  ...
  const getUserProfile = useBoundStore((state) => state.getUserProfile);

  const getUserProfileStatus = useBoundStore((state) => state.getUserProfileStatus);

  useEffect(() => {
    if (profileId) {
      getUserProfile(profileId);
      getUserAttendingEvents(profileId);
      getUserHostingEvents(profileId);
    }
  }, []);

  if (userProfile === undefined || getUserProfileStatus) {
    return (
  ...
  return (
    <div className="flex h-full flex-col overflow-visible md:rounded-lg">
      <div className="relative flex h-10 w-full bg-green-500 md:h-16 md:rounded-t-lg">
        <img
          src={isImageError ? userPlaceholderImage : userProfile.avatar || undefined}
      ...
      <div className="mt-10 overflow-visible">
        <h1 className="px-4 pb-4 text-xl font-bold md:text-3xl">{userProfile.full_name}</h1>
        <p className="px-4 pb-1 font-medium">Bio</p>
        <p className="border-b border-green-200 px-4 pb-4">{userProfile.bio}</p>
